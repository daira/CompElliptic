...
the kernel's. `partial def` breaks the premiss: the kernel constant is `opaque` with no body, and
...
These declarations live in the test-only library for the same reason as `MetaCheck.AxiomCheck` and
`MetaCheck.CompiledOverride`: the production `CompElliptic` import graph must never reach them.
This is a module of its own so the closure-wide compiled-body sweep, which the sibling file
deliberately trips, does not mask the checks pinned here.
-/

namespace MetaCheck.NativeSurface
...
assert_axioms MetaCheck.NativeSurface.honestMatch +native(
  MetaCheck.NativeSurface.honestMatch)
...
assert_axioms MetaCheck.NativeSurface.partialMatch +native(
  MetaCheck.NativeSurface.partialMatch)
...
/-- error: MetaCheck.NativeSurface.capturedPartial is not a def -/
#guard_msgs (whitespace := lax) in
assert_computable MetaCheck.NativeSurface.capturedPartial

/-! ## A sealed `opaque` is not reported

The reduction-barrier idiom: the opaque's *type* carries the equation to its source, so the kernel
can recover the value, and its compiled body is the declared one. The certificate below is
therefore honest, and the check leaves it alone. -/
...
assert_axioms MetaCheck.NativeSurface.sealedMatch +native(
  MetaCheck.NativeSurface.sealedMatch)
...
`csimpSlow`, and a walk of the statement alone would never reach `hiddenBehindCsimp`. This is the
routing the fast lane anticipates: `CompElliptic.Curves.Pasta.Fast.Projective` keeps the proven
equality `padd_eq_paddFast` for a future registration, and the replacement target's cone is where
a `partial` would sit unnoticed.
...
assert_axioms MetaCheck.NativeSurface.csimpSlow_eq_csimpFast
...
#guard_msgs (whitespace := lax) in
assert_axioms MetaCheck.NativeSurface.csimpCert +native(
  MetaCheck.NativeSurface.csimpCert)

end MetaCheck.NativeSurface
