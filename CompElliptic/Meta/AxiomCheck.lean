...
/-!
# `assert_axioms` and `assert_computable` — concise, build-checked trust assertions

Siblings of Mathlib's `assert_no_sorry` (same `collectAxioms` machinery) that assert an *upper
bound* on a declaration's trusted base. Unlike a `#guard_msgs`-pinned `#print axioms`, they do not
hard-code the pretty-printed axiom list, so they stay green across toolchain bumps that rename the
...
about what compiled code runs, and the census's `+native` entries are `native_decide` proofs that
run it. Two sections below cover the two ways compiled behaviour comes apart from kernel meaning —
a substituted body (`@[implemented_by]`, `@[extern]`) and a `partial` declaration inside a
certificate's evaluated term.

Scope: these commands catch *inadvertent* drift — an entry silently reaching more than it
claims. They are not a full defence against a deliberately deceptive author. The provenance
...
These commands are also used in ironwood (`Zcash/Meta/AxiomCheck.lean`); changes here should be
...
attacker's. That matters here because the census's `+native` entries are `native_decide` proofs,
which *run the compiled bodies* — a doctored body makes such a certificate report a fact that the
kernel value refutes.

The check below is deliberately not cone-scoped. `collectAxioms` no longer walks imported bodies
(it reads a per-module precomputed footprint), so a transitive walk over a certificate's cone would
be the expensive traversal Lean stopped doing — and what it would mostly find is the *ambient*
surface: at the pinned toolchain, `Init`/`Std`/`Lean`/`Mathlib` carry on the order of a thousand of
these attributes between them, all of them compiled code every `native_decide` in existence already
runs. So the census splits the two rather than walking terms. On the ambient surface it claims
nothing new. Outside it, it enumerates *every* override in the import closure, reachable or not:
the attribute extensions are indexed by declaring module, so this is a scan of the module list
rather than of any term, and it catches an override before the commit that puts it in a cone.
Stating it over the whole closure is sound because `ParametricAttribute`'s `add` refuses a
declaration from an imported module (`Lean.Attributes`, `throwAttrDeclInImportedModule`) — a commit
can only attach either attribute inside the module that declares the constant, so every override a
commit here can introduce lives in a module this repository owns, and none of them can hide behind
an ambient root. -/

/-- Package roots whose compiled-body overrides are *ambient*: the Lean toolchain and the pinned
dependency stack (CompPoly and the mathlib stack it version-pins), whose compiled code every
`native_decide` runs no matter what this repository does, and which a commit here cannot add an
override to (attributes cannot be attached to an imported declaration). Everything else is
censused, so a *new* dependency fails the build until its root is classified — a new package in
the import closure is a trusted-base change, unlike a version bump of one already listed. This is
where the `@[extern]` `Task.spawn` / `Task.get` reached through
`CompElliptic.Curves.Pasta.Fast.Msm.parMap` land: the kernel sees their reference bodies, and
`parMap_eq_map` closes by `rfl`. -/
def ambientPackageRoots : Array Name :=
  #[`Init, `Lean, `Std, `Lake, `Batteries, `Mathlib, `Aesop, `Qq, `Plausible, `ProofWidgets,
    `ImportGraph, `LeanSearchClient, `Cli, `CompPoly]
...
`CompElliptic.Curves.Pasta.Fast.Projective` records the convention that a kernel-checked equality
(`padd_eq_paddFast`, registrable as `@[csimp]`) is used instead.
...
the census as a compiler-trust certificate. `MetaCheck.AxiomCheck.MacroForged` pins the rejection.
A genuine auxiliary is always emitted by elaborating syntax *within* the owner's declaration — a
tactic in its proof body, or an auto-param inside a structure instance — so in both shapes its
start is strictly after the owner's first token. -/
...
`@[csimp]` lemma replaces a constant wholesale in compiled code, so a certificate whose statement
says `padd` would run `paddFast` under such a registration. The walk therefore follows those
replacements. Without that step a `partial` reachable only through a replacement target would be
invisible here; this repository registers no `@[csimp]` today, but keeps the proven equality
(`CompElliptic.Curves.Pasta.Fast.Projective.PVes.padd_eq_paddFast`) for a future registration,
which the walk's following of the map already covers. That step is owed entirely to `@[csimp]`
being permitted: prohibiting it retires the replacement map, and the walk collapses back to
following the term the axiom states.

Plain `opaque` is deliberately not reported. Its compiled body *is* its declared body, so it cannot
make a certificate disagree with the source; and the sealed-subtype idiom
(`opaque h : {x // x = source} := ⟨source, rfl⟩`, whose acceptance `MetaCheck.NativeSurface` pins)
keeps the value kernel-recoverable through the type. An `opaque` that *is* body-substituted carries
`@[implemented_by]` and is caught by the sweep above. -/

/-- Whether `decl` is the `opaque` constant a `partial def` leaves behind, recognized by the
...
have evaluated — the certificate becomes a statement about an abstract constant backed by an
...
evaluated a `partial` declaration (`checkNativeEvaluatedSurface`); `MetaCheck.NativeSurface`
pins the latter.
-/
...
kernel. `MetaCheck.AxiomCheck.ComputableSafety` pins the rejection.
...
`checkCompiledBodyDisclosure`); `MetaCheck.CompiledOverride` pins both rejections.
-/
